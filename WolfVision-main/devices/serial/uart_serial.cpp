...
SerialPort::SerialPort(std::string _serial_config) {
  ...
  const char* DeviceName[] = {"/dev/ttyS0", "/dev/ttyTHS2", "/dev/ttyUSB2", "/dev/ttyUSB3"};

  struct termios newstate;
  bzero(&newstate, sizeof(newstate));

  fd = open(DeviceName[0], O_RDWR | O_NONBLOCK | O_NOCTTY | O_NDELAY);
  if (fd == -1) {
    fmt::print("[{}] Open serial device failed: {}\n", idntifier_red, DeviceName[0]);
  } else {
    fmt::print("[{}] Open serial device success: {}\n", idntifier_green, DeviceName[0]);
  }
  fd2 = open(DeviceName[1], O_RDWR | O_NONBLOCK | O_NOCTTY | O_NDELAY);
  if (fd2 == -1) {
    fmt::print("[{}] Open serial device failed: {}\n", idntifier_red, DeviceName[1]);
  } else {
    fmt::print("[{}] Open serial device success: {}\n", idntifier_green, DeviceName[1]);
  }
  ...
  tcsetattr(fd2, TCSANOW, &newstate);
}

SerialPort::~SerialPort(void) {
  if (!close(fd)) { fmt::print("[{}] Close serial device success: {}\n", idntifier_green, fd); }
  if (!close(fd2)) { fmt::print("[{}] Close serial device success: {}\n", idntifier_green, fd2); }
}
...
void SerialPort::receiveData() {
  ...
  read_message_ = read(fd2, receive_buff_temp_, sizeof(receive_buff_temp_));
...
void SerialPort::updateReceiveInformation() {
  ...
  // for (size_t i = 0; i != sizeof(transform_arr_) / sizeof(transform_arr_[0]); ++i) {
    transform_arr_[0] = receive_buff_[1] - '0';
  // }
  ...
  receive_data_.bullet_velocity = 26;
}
